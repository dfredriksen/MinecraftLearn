...
class LearningThread(Thread):
  ...
  def __init__(self,replay_memory, state, action, next_state, reward, steps_done, environment, episode, learning_threads):
  ...
  def run(self):
    ...
    reward = self.memory.calculate_reward(self.state, self.reward, np.array(im))
